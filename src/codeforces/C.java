...
public class C {
  ...
  static class Solver {
      ...
      void solve() {
          long numberOfPotionsToMake = io.nextLong();
          int countSpellsType1 = io.nextInt();
          int countSpellsType2 = io.nextInt();
          long timePerPotion = io.nextLong();
          long initialMana = io.nextLong();

          long[] discount1 = new long[countSpellsType1+1];
          for (int i=0; i<countSpellsType1; i++) {
              discount1[i] = io.nextInt();
          }
          discount1[countSpellsType1] = timePerPotion;
          long[] mana1 = new long[countSpellsType1+1];
          for (int i=0; i<countSpellsType1; i++) {
              mana1[i] = io.nextInt();
          }

          long[] numb2 = new long[countSpellsType2+1];
          for (int i=1; i<=countSpellsType2; i++) {
              numb2[i] = io.nextLong();
          }
          long[] mana2 = new long[countSpellsType2+1];
          for (int i=1; i<=countSpellsType2; i++) {
              mana2[i] = io.nextLong();
          }

          long bestAns = Long.MAX_VALUE;
          for (int i=0; i<discount1.length; i++) {
              if (mana1[i] > initialMana) continue;
              long mana = initialMana - mana1[i];
              long extras2 = binSearch(numb2, mana2, mana);
              long leftOvers = numberOfPotionsToMake - extras2;
              long timeForLeftOvers = leftOvers * discount1[i];
              bestAns = Math.min(bestAns, timeForLeftOvers);
          }

          io.println(bestAns);
      }

      long binSearch(long[] numb2, long[] mana2, long mana) {
          int min = 0;
          int max = numb2.length-1;
          while (min < max) {
              int guess = (min+max+1)/2;
              boolean possible = (mana2[guess] <= mana);
              if (possible) {
                  min = guess;
              } else {
                  max = guess-1;
